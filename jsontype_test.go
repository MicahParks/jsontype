...
func TestUUID(t *testing.T) {
	const rawUUID = "0ffc3b28-dcdd-4c4d-a965-b5ec866c6262"
	validJSON := json.RawMessage(encloseDoubleQuotes(rawUUID))
	...
	// Success marshal.
	{
		...
		if !bytes.Equal(b, validJSON) {
			t.Fatalf("invalid JSON produced for UUID")
		}
	}
	...
	// Success unmarshal.
	unmarshal := jsontype.New(uuid.UUID{})
	{
		err = json.Unmarshal(validJSON, &unmarshal)
		if err != nil {
			t.Fatalf("failed to unmarshal struct: %s", err)
		...
		if unmarshal.Get().String() != u.String() {
			t.Fatalf("invalid UUID unmarshalled")
		}
	}
